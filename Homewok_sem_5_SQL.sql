DROP DATABASE IF EXISTS homework_sem_5;
CREATE DATABASE homework_sem_5;
USE homework_sem_5;
-- пользователи
...
DROP VIEW IF EXISTS user_young;
CREATE VIEW user_young AS
SELECT id, firstname, lastname, hometown, gender
From users u_1
JOIN  profiles pr_1
ON u_1.id=pr_1.user_id
WHERE id IN (
	SELECT user_id 
	FROM profiles
	WHERE TIMESTAMPDIFF(YEAR, birthday, NOW()) < 20);
    
SELECT * FROM user_young;
...
SELECT 
	...
	firstname AS 'Имя', lastname AS 'Фамилия'
FROM users u
...
SELECT *, 
	 LAG(created_at, 1, 0) OVER w AS 'Lag',
     LEAD(created_at) OVER w AS 'Lead'
FROM messages
WINDOW w AS (PARTITION BY TIMESTAMPDIFF(MINUTE, created_at, created_at) ORDER BY TIMESTAMPDIFF(MINUTE, created_at, NOW()) DESC);
...
JOIN age_user
ON age_user.user_id = media.user_id
WHERE age < 12;

-- 2. Определить кто больше поставил лайков(всего): мужчины или женщины.
SELECT gender, COUNT(likes.user_id) AS 'Количество лайков'
FROM profiles 
JOIN likes
